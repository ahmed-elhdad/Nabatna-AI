...
from src.models.enums import AuthEnum
from src.models import UserDataModel
...
def _get_current_user(
    request: Request, app_settings: Settings = Depends(get_settings)
) -> dict:
...
async def login(
    ...
    if stored_user is None or not verify_password(
        request_body.password, stored_user.get("password", "")
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
